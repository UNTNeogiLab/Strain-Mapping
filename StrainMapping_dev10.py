...
print('Parallelizing')
paramsarray = np.zeros((x.shape[1]//N, x.shape[2]//N, 8))
...
def vplot(paramsarray):
    f , ((ax1,ax2),(ax3,ax4),(ax5,ax6)) = plt.subplots(3,2,
        sharex=True,sharey=True,dpi=200)
    A_v = paramsarray[:,:,4]
    B_v = paramsarray[:,:,5]
    Delta_v = np.mod(paramsarray[:,:,6])#*60/(np.pi),60)
    Theta_v = np.mod(paramsarray[:,:,7])#*60/(np.pi),60)
    ...
    exx_v = 0.5*((A_v-2*chi)/((1-nu)*(p1+p2)) + B_v/((1+nu)*(p1-p2)))
    eyy_v = 0.5*((A_v-2*chi)/((1-nu)*(p1+p2)) - B_v/((1+nu)*(p1-p2)))


    f1 = ax1.imshow(A_v, cmap='jet')
    f2 = ax2.imshow(B_v, cmap='jet')
    f3 = ax3.imshow(Delta_v,cmap='jet')
    f4 = ax4.imshow(Theta_v,cmap='jet')
    f5 = ax5.imshow(exx_v,cmap='jet')
    f6 = ax6.imshow(eyy_v,cmap='jet')
    
    fs = 10
    
    ax1.set_title(r'$A_v$', fontsize= fs)
    f.colorbar(f1, ax=ax1)
    ax2.set_title(r'$B_v$', fontsize= fs)
    f.colorbar(f2, ax=ax2)
    ax3.set_title(r'$\delta_v (Orientation)$', fontsize= fs)
    f.colorbar(f3, ax=ax3)
    ax4.set_title(r'$\theta_v (Strain angle)$', fontsize= fs)
    f.colorbar(f4, ax=ax4)
    ax5.set_title(r'$e_x$$_x_v $', fontsize= fs)
    f.colorbar(f5, ax=ax5)
    ax6.set_title(r'$e_y$$_y_v $', fontsize= fs)
    f.colorbar(f6, ax=ax6)


    plt.tight_layout()
    plt.savefig(str(directory) + '_var.png', dpi=2000)    
pplot(outfile)
vplot(outfile)
#plt.show()
